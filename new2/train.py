...
def train_and_evaluate(train_loader, val_loader, model, criterion, optimizer, device, epoch_num, start_epoch):
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        if val_kappa > best_model_info['kappa']:
            ...
            best_model_path = os.path.join(args.model_path, f'{args.model}_epoch{epoch}.pth')
            torch.save(best_model_info['state_dict'], best_model_path)

    final_model_path = os.path.join(args.model_path, f'{args.model}_epoch{epoch}.pth')
...
if __name__ == '__main__':
    ...
    train_and_evaluate(train_loader, val_loader, model, criterion, optimizer, device, args.num_epochs, args.resume_from_epoch + 1)                  